...
4. Keep auto-publish disabled. Review the GTABase HTML sources added by migration `202609100016_configure_weekly_vehicle_sources.sql`, including publisher permission/robots guidance, then run each source test. Check real parsed titles/URLs and rejection counts; a successful fetch does not prove extraction or publication. Weekly collection is limited to Thursdays after the configured UTC hour.
