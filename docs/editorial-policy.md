...
In the same operator-reported check, the [official GTA VI page](https://www.rockstargames.com/VI) was accessible and displayed November 19, 2026. This is a dated report of source availability, not a seeded application fact or a verified ingestion result. The migrations leave `release_date` null. An administrator must revisit the official page, confirm the current statement, then configure `release_date`, `release_source_url`, and `release_verified_at` in settings. Preserve date/time semantics and update the value if Rockstar changes its announcement.
